...
class Repository(
    ...
    suspend fun syncWithSupabase() {
        try {
            ...
            var remoteCats = supabase.getCategories()
            if (remoteCats.isEmpty()) {
                Log.d(TAG, "Supabase category table is empty. Seeding defaults with beautiful images...")
                ...
                // Post to Supabase asynchronously so it persists remotely and becomes real
                for (cat in defaultCategories) {
                    try {
                        supabase.createCategory(cat)
                    } catch (e: Exception) {
                        Log.e(TAG, "Failed to upload default category: ${e.message}")
                    }
                }
                remoteCats = defaultCategories
            } else {
                categoryDao.clearAll()
                categoryDao.insertCategories(remoteCats)
            }
            Log.d(TAG, "Synced ${remoteCats.size} categories with database.")
        } catch (e: Exception) {
        ...
        try {
            ...
            var remoteProviders = supabase.getServiceProviders()
            if (remoteProviders.isEmpty()) {
                Log.d(TAG, "Supabase provider table is empty. Seeding defaults...")
                ...
                for (prov in defaultProviders) {
                    try {
                        ...
                        Log.e(TAG, "Failed to upload default provider: ${e.message}")
                    }
                }
                remoteProviders = defaultProviders
            } else {
                serviceProviderDao.clearAll()
                serviceProviderDao.insertServiceProviders(remoteProviders)
            }
            Log.d(TAG, "Synced ${remoteProviders.size} service providers from database.")
        } catch (e: Exception) {
